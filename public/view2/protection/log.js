define(function(require) {
    var tpl = require('text!protection/log.html');
    ...
    var op = {
        ...
        columns: {
            ...
            rfw_ip: {
                '0': [{
                    ...
                    formatter: function(value, row, idx) {
                        ...
                        return '<div>' + value + ':' + row.srcport + '</div>';
                    }
                }, {
                    field: 'dstaddr',
                    title: '黑客',
                    ...
                    formatter: function(value, row, idx) {
                        ...
                        return '<div>' + value + ':' + row.dstport + '</div>';
                    }
                }, {
                    field: 'result',
                    title: '状态',
                    align: 'center',
                    sortable: true,
                    formatter: function(value, row, iidx) {
                        return '<div>' + (value == 0 ? '放行' : '阻止') + '</div>';
                    }
                }, {
                    field: 'rulename',
                    title: '拦截原因',
            ...
            rfw_net: {
                '0': [{
                    ...
                    sortable: true,
                    formatter: function(value, row, index) {
                        return '<div>' + (value == 401 ? '连接' : '监听') + '</div>';
                    }
                },{
                    field: 'result',
                    title: '允许联网',
            ...
            rfw_share: {
                '0': [{
                    ...
                    sortable: true,
                    formatter: function(value, row, index) {
                        var act = ['', '创建文件', '创建目录', '访问文件', '修改文件', '删除文件', '删除文件夹', '重命名', '移动文件', '访问拒绝'];
                        if (value == undefined) {
                            return '';
                        }
                        return '<div>' + act[value] + '</div>';
                    }
                }, {
                    field: 'name',
                    title: '共享文件',
            ...
            rfw_term: {
                '0': [{
                    ...
                    formatter: function(value, row, index) {
                        ...
                        return '<div>' + value + '-' + row.endtime + '</div>';
                    }
                }, {
                    field: 'updateflow',
                    title: '上传流量',
                    ...
                    formatter: function(value, row, idx) {
                        ...
                        return '<div>' + op.formatDiskSize(value) + '</div>';
                    }
                }, {
                    field: 'downloadflow',
                    title: '下载流量',
        ...
        init: function(container, type, first) {
            ...
            op._type = type || params['l_rfwType'] || 'rfw_url';
            var dataobj = {};
        ...
        getParams: function(view, type, params) {
            ...
            if (type == 'rfw_net') {
                ...
                if(params['l_act']){
                    view.find('.js_task a').removeClass('active');
                    view.find('.js_task [val=' + params['l_act'] + ']').addClass('active');
                }
        ...
        setParams: function(view, type, params) {
            ...
            if (type == 'rfw_url') {
                ...
                _params['l_task'] = params.queryconditions.result;
            ...
            if (type == 'rfw_ip') {
                _params['l_startTime'] = params.queryconditions.begintime;
                _params['l_endTime'] = params.queryconditions.endtime;
                _params['l_state'] = params.queryconditions.result;
            }
            if (type == 'rfw_ad') {
                _params['l_startTime'] = params.queryconditions.begintime;
                _params['l_endTime'] = params.queryconditions.endtime;
            ...
            if (type == 'rfw_share') {
                _params['l_startTime'] = params.queryconditions.begintime;
                _params['l_endTime'] = params.queryconditions.endtime;
                _params['l_task'] = params.queryconditions.action;
                _params['l_stype'] = params.queryconditions.searchtype;
                _params['l_stxt'] = params.queryconditions.searchkey;
            }
            if (type == 'rfw_term') {
        ...
        get_rfw_url: function(params, view) {
            ...
            params.queryconditions.result = view.find('.js_task .active').attr('val');
            return params;
        },
        get_rfw_ip: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            params.queryconditions.result = view.find('.js_state .active').attr('val');
            return params;
        },
        get_rfw_ad: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            params.queryconditions.searchkey = view.find('.js_searchKey').val();
        ...
        get_rfw_brow: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            params.queryconditions.searchkey = view.find('.js_searchKey').val();
        ...
        get_rfw_net: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            params.queryconditions.result = view.find('.js_result .active').attr('val');
            params.queryconditions.action = view.find('.js_task .active').attr('val');
            params.queryconditions.searchkey = view.find('.js_searchKey').val();
            params.queryconditions.searchtype = view.find('.js_searchType').val();
            return params;
        },
        get_rfw_share: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            params.queryconditions.searchkey = view.find('.js_searchKey').val();
        ...
        get_rfw_term: function(params, view) {
            var time = this.getDate(view);
            params.queryconditions.begintime = time.begintime;
            params.queryconditions.endtime = time.endtime;
            return params;
        ...
        initTable: function(view, columns, showSearch, first) {
            ...
            var urls = {
                'rfw_url': 'rfwlog/urlintercept',
                'rfw_ip': 'rfwlog/ipaccess',
                'rfw_ad': 'rfwlog/sharedreslist',
                'rfw_brow': 'rfwlog/browsing',
                'rfw_net': 'rfwlog/netproc',
                'rfw_share': 'rfwlog/sharedresaccess',
                'rfw_term': 'rfwlog/terminalflow'
            };
        ...
        initEvent: function(view) {
            ...
            view.on('click', '#custom-type a', function() {
                ...
                op.tempParams.viewtype = that.attr('val');
                if (logType == 'rfw_brow') {
                    if (viewType == 'ep') {
                        op.query[logType].paging.sort = '';
                    } else {
