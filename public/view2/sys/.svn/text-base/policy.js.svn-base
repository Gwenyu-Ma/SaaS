define(function(require) {
    ...
    var op = {
        ...
        bindEvent: function(container) {
            ...
            container.on('click', '.js_rule_del', function() {
                ...
                obj.fadeOut(400, function() {
                    obj.remove();
                    fa.find('.js_and:eq(0)').css({ 'visibility': 'hidden' });
                });
            });
      ...
      return xml;
        ...
        ruleBox: function(id, name, _html, flag) {
            ...
            $html.find('.js_and:eq(0)').css({ 'visibility': 'hidden' });
        ...
        toHtml2: function(container, json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                if (!RsCore.cache.group.list[groupid]) { continue; }
                len++;
    ...
    return {
        ...
        render: function(container) {
            ...
            var view = '';
            if (groupId != '0') {
                view = mustache.render(tpl, { noGoble: true });
                op.noGoble = true;
            } else {
                view = mustache.render(tpl, { noGoble: false });
                op.noGoble = false;
            }
        ...
        destroy: function() {
            ...
            $(this.container).find('.c-moudle-wrap').slimscroll({ destroy: true });
