define(function(require) {
    ...
    var defaultSettingData = {
        order: ['threaten', 'os', 'virCount', 'virKill', 'flow', 'network', 'interception', 'location', 'maliciousUrl'],
        threaten: {
            moduleName: 'threaten',
            width: 2,
            isShow: 1
    ...
    var op = {
        ...
        initEchart: function(container) {
            ...
            /*pc*/
            var os_pc_Obj = view.find('#client-pc')[0];
            var os_pc_Chart = echarts.init(os_pc_Obj);
            os_pc_Chart.showLoading();
            // require(['../test-data/client'], function(setting) {
            //     os_pc_Chart.hideLoading();
            //     os_pc_Chart.setOption(setting);
            // });
            this.echartsObj.push(os_pc_Chart);
            os_pc_Chart.on('click', function(params) {
                if(params.name.indexOf('离线')>-1){
                    window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=windows&o_online=0';
                }else if(params.name.indexOf('在线')>-1){
                   window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=windows&o_online=1';
                }else{
                    window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=windows';
                }
                
            });
            /*mobile*/
            var os_mobile_Obj = view.find('#client-mobile')[0];
            var os_mobile_Chart = echarts.init(os_mobile_Obj);
            os_mobile_Chart.showLoading();
            // require(['../test-data/mobile'], function(setting) {
            //     os_mobile_Chart.hideLoading();
            //     os_mobile_Chart.setOption(setting);
            // });
            os_mobile_Chart.on('click', function(params) {
                if(params.name.indexOf('离线')>-1){
                    window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=android&o_online=0';
                }else if(params.name.indexOf('在线')>-1){
                    window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=android&o_online=1';
                }else{
                    window.location.hash = 'sys' + '/overview?g=0&vopen=1&o_stype=sys&o_stxt=android';
                }
                
            });
            this.echartsObj.push(os_mobile_Chart);
            ...
            RsCore.ajax('Sys/getEpOnlineStatistics', function(data) {
                ...
                view.find('.client-info-num em').text(total);
                os_pc_Chart.hideLoading();
                os_pc_Chart.setOption(myEchartSet.initPc(data));
                os_mobile_Chart.hideLoading();
                os_mobile_Chart.setOption(myEchartSet.initMob(data));
            });
        ...
        getAndroidQr: function(getQrTime, view) {
            $.get('/index.php/index/package', { platform: 'android' }, function(json) {
                json = $.parseJSON(json);
                if (json && json.r) {
                    var r = json.r;
                    switch (r.code) {
                        ...
                        case 0: //成功                                
                            ...
                            view.find('.down-rwm img:eq(0)').prop('src', json.data.dist.qr);
        ...
        initEvent: function(container) {
            ...
            view.on('mouseenter', '.js_reload_rwm', function() {
                $(this).find('img:eq(0)').hide().end().find('img:eq(1)').show();
            }).on('mouseleave', '.js_reload_rwm', function() {
                $(this).find('img:eq(1)').hide().end().find('img:eq(0)').show();
            ...
            view.on('click', '.down-sys a', function() {
                ...
                me.addClass('process');
                $.get('/index.php/index/package', { platform: type }, function(json) {
                    json = $.parseJSON(json);
                    ...
                    if (json && json.r) {
                        ...
                        switch (r.code) {
                            ...
                            case 0: //成功                                
                                //window.open(json.data.dist.link);
                                $downBox.find('.down-box-bod').html(json.data.dist.link);
                                $downBox.find('.down-box-tit h3').text('安装包下载地址');
            ...
            function homeResize() {
                view.find('.home-wrap').slimScroll({
                    height: view.height(),
                    size: '4px',
                    alwaysVisible: true
                });
        ...
        createEchart: {
            threaten: function() {
                ...
                //     threat_Chart.setOption(setting);
                // });
                op.echartsObj.push(threat_Chart);
                ...
                RsCore.ajax('Xavlog/getclientperilstatistics', function(data) {
                    ...
                    avg = (avg > 0 && avg < 1) ? '小于1' : avg.toFixed(0);
                    ...
                    threat_Chart.on('click', function(params) {
                        ...
                        if(xavType == '病毒'){
                            window.location.hash = 'virus' + '/log?g=0&vopen=1&l_xavType=xav_virus&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }
                        if(xavType == '网址'){
                            window.location.hash = 'protection' + '/log?g=0&vopen=1&l_rfwType=rfw_url&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }
                        if(xavType == '骚扰'){
                            window.location.hash = 'mobile' + '/log?g=0&vopen=1&l_xavType=mb_msg&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }
                        if(xavType == '联网'){
                            window.location.hash = 'protection' + '/log?g=0&vopen=1&l_rfwType=rfw_brow&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }
            ...
            os: function() {
                ...
                //     system_Chart.setOption(setting);
                // });
                op.echartsObj.push(system_Chart);
            ...
            virCount: function() {
                ...
                //     virus_Chart.setOption(setting);
                // });
                op.echartsObj.push(virus_Chart);
                RsCore.ajax('Xavlog/getXavStatistics', function(data) {
                    ...
                    var now = Hour + ':' + Minute;
                    /*事件*/
                    $(virus_Obj).closest('.virus-box').off('click').on('click', '.echart_opt li', function() {
                        var that = $(this),
                            type = that.attr('data-type'),
                            info = that.closest('.virus-box').find('.echart_info');
                        ...
                        info.html(_tpl);
                    }).find('.echart_opt li:eq(0)').trigger('click');
                    virus_Chart.on('click', function(params) {
                        ...
                        window.location.hash = 'virus' + '/log?g=0&vopen=1&l_xavType=xav_virus&l_view=0&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                    });
                });
            },
            virKill: function() {
                ...
                //     virus2_Chart.setOption(setting);
                // });
                op.echartsObj.push(virus2_Chart);
                ...
                RsCore.ajax('Xavlog/getXavStatistics', function(data) {
                    ...
                    var now = Hour + ':' + Minute;
                    /*事件*/
                    $(virus2_Obj).closest('.virus2-box').off('click').on('click', '.echart_opt li', function() {
                        var that = $(this),
                            type = that.attr('data-type'),
                            info = that.closest('.virus2-box').find('.echart_info');
                        that.addClass('active').siblings().removeClass('active');
                    ...
                    virus2_Chart.on('click', function(params) {
                        ...
                        window.location.hash = 'virus' + '/log?g=0&vopen=1&l_xavType=xav_virus&l_view=2&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                    });
                });
            },
            flow: function() {
                ...
                // var flow_Chart = echarts.init(flow_Obj);
                // flow_Chart.showLoading();
                // require(['../test-data/flow'], function(setting) {
                //     flow_Chart.hideLoading();
                //     flow_Chart.setOption(setting);
                // });
                // op.echartsObj.push(flow_Chart);
                // var html = '<div id="home-flow">A</div>';

                // flow_Chart.on('mouseover', function(params) {
                //     console.log(params);
                // });
                // flow_Chart.on('mouseout', function() {
                //     $('#home-flow').remove();
                // });
                ...
                var html_temp = '<div class="home-flow">' +
                    '<span class="num">{{num}}</span>' +
                    '<div class="home-flow-box" style="width:{{w}}%">' +
                    '<div><i class="home-flow-box-left"></i><i class="home-flow-box-right"></i>{{name}}</div>' +
                    '<span class="flow-up" style="width:{{upw}}%"><i>{{up}}</i></span><span class="flow-down"><i>{{down}}</i></span></div></div>';
                var html_arr = [];
                RsCore.ajax('rfwlog/getFlowAudit', function(datas) {
                    ...
                    if (num == 1) {
                        ...
                        _html = html_arr.slice(0, 6).join('');
                    }
                    _html = '<div class="flow-sort-box">' + _html + '</div>';
                    _html += '<div class="flow-fotter"><span class="flow-foter-up">上行流量</span><span class="flow-foter-down">下行流量</span></div>';
                    flow_Obj.html(_html);
                    view.find('.home-flow:eq(0)').addClass('active');
                    view.find('.home-flow').on('mouseover', function() {
                        view.find('.home-flow').removeClass('active');
                    });
                });
            ...
            network: function() {
                ...
                RsCore.ajax('rfwlog/getURLAudit', function(data) {
                    ...
                    network_Chart.on('click', function(params) {
                        ...
                        if(Type=='禁止网址'){
                            ...
                            window.location.hash = 'protection' + '/log?g=0&vopen=1&l_rfwType=rfw_net&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }else if(Type=='共享访问'){
                            window.location.hash = 'protection' + '/log?g=0&vopen=1&l_rfwType=rfw_share&l_view=1&topen=1&l_time=special&l_startTime=' + startTime + '&l_endTime=' + startTime;
                        }
                ...
                //     network_Chart.hideLoading();
                //     network_Chart.setOption(setting);
                // });
                op.echartsObj.push(network_Chart);
            },
            interception: function() {
                ...
                //     interception_Chart.setOption(setting);
                // });
                op.echartsObj.push(interception_Chart);
            },
            location: function() {
                /*终端定位*/
            ...
            maliciousUrl: function() {
                ...
                // require(['../test-data/site'], function(setting) {
                //     site_Chart.hideLoading();
                //     site_Chart.setOption(setting);
                // });
                op.echartsObj.push(site_Chart);
            }
        },
        DownloadEvt: null,
            ...
            if (ua.indexOf('MSIE') > 0 || !!ua.match(/Trident.*rv\:11\./)) {
                ...
                if (DownloadLink) {
                    ...
                    if (document.createEvent) {
                        ...
                        op.DownloadEvt.initEvent('click', true, false);
        ...
        customModule: function(opt, view) {
            ...
            $li.each(function(index) {
                ...
                if (data[moduleName].width == 1) {
                    $width.eq(0).addClass('active');
                } else {
                    $width.eq(1).addClass('active');
                }
            ...
            $echLi.each(function(index) {
                ...
                if (filter.indexOf(moduleName) == -1) {
                    if (data[moduleName].width == 1) {
                        $echLi.eq(index).removeClass('echarts-w2').addClass('echarts-w1');
                        $echLi.eq(index).removeAttr('style');
                        $width.eq(0).addClass('active');
                    } else {
                        $echLi.eq(index).removeAttr('style');
                        $echLi.eq(index).removeClass('echarts-w1').addClass('echarts-w2');
                        $width.eq(1).addClass('active');
                    }
                }
            ...
            //设置图表可拖拽
            $('.echarts-list').dragsort({
                dragSelector: '.echarts-list h3',
                itemSelector: '.echarts-list > li',
                dragBetween: true,
                dragEnd: saveOrder,
                placeHolderTemplate: '<li class="placeDrag"></li>'
            });

            view.off('click', '.custom-mod').on('click', '.custom-mod', function(e) {
                $(this).addClass('click');
                ...
                view.on('click', '.custom-mod-box', function(e) {
                    ...
                    var locationModule = $(target).parents('li').data('module');
            ...
            function saveOrder() {
                ...
                data.order = [];
                $li.each(function(index) {
                    data.order.push($li.eq(index).data('mod'));
                });
            ...
            function applySetting(opt) {
                ...
                RsCore.ajax('/user/setusersetting', { setStr: JSON.stringify(opt) }, function(data, code, msg) {});
                $customModBox.hide();
            ...
            function defaultSetting() {
                $customShowBtn.removeClass('click');
                $customModBox.hide();
                RsCore.ajax('/user/setusersetting', { setStr: JSON.stringify(defaultSettingData) }, function(data, code, msg) {});
        ...
        messageCenter: function(view) {
            ...
            function renderHTML(keyword) {
                ...
                RsCore.ajax('message/getmsg', data, function(data) {
                    var html = '';
                    var type = ['rs', 'pf', 'am', 'rp', 'b'];
                    if (typeof keyword != 'undefined') {
                        if (!data.messages.length) {
                            html = '<div class="home-msg-nomatch"><p>抱歉，关键词“<span>' +
                                keyword + '</span>”未搜到内容，请尝试其他关键字~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                            $('.home-msg>.home-bod>div').html(html);
                            return;
                        }
                    } else {
                        if (!data.messages.length) {
                            html = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                            $('.home-msg>.home-bod>div').html(html);
                            return;
                        }
                    }
                    ...
                    // resTransform.reverse();

                    //假数据
                    for (var j = 0; j < resTransform.length; j++) {
                        var rtype = resTransform[j].types[0].split(':')[0];
                    ...
                    if (sysType.length != 0) {
                        sysTpl = '<ul class="msg-list">{{#item}}<li><i class="' + '{{types}}' + '">&nbsp;</i><div class="msg-list-txt"><div><a data-id="{{_id.$id}}"  data-did="{{did.$id}}">{{msgtitle}}{{#isread}}<i class="new">新</i>{{/isread}}</a></div><span>{{outtime}}</span><i class="msgdel" data-id="{{did.$id}}"></i></div></li>{{/item}}</ul><a href="/Manage/index#settings/message?g=00">查看更多</a>';
                    } else {
                        sysTpl = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                    }
                    if (secType.length != 0) {
                        secTpl = '<ul class="msg-list">{{#item}}<li><i class="' + '{{types}}' + '">&nbsp;</i><div class="msg-list-txt"><div><a data-id="{{_id.$id}}"  data-did="{{did.$id}}">{{msgtitle}}{{#isread}}<i class="new">新</i>{{/isread}}</a></div><span>{{outtime}}</span><i class="msgdel" data-id="{{did.$id}}"></i></div></li>{{/item}}</ul><a href="/Manage/index#settings/message?g=00">查看更多</a>';
                    } else {
                        secTpl = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                    }
                    if (logType.length != 0) {
                        logTpl = '<ul class="msg-list">{{#item}}<li><i class="' + '{{types}}' + '">&nbsp;</i><div class="msg-list-txt"><div><a data-id="{{_id.$id}}"  data-did="{{did.$id}}">{{msgtitle}}{{#isread}}<i class="new">新</i>{{/isread}}</a></div><span>{{outtime}}</span><i class="msgdel" data-id="{{did.$id}}"></i></div></li>{{/item}}</ul><a href="/Manage/index#settings/message?g=00">查看更多</a>';
                    } else {
                        logTpl = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                    }
                    if (facType.length != 0) {
                        facTpl = '<ul class="msg-list">{{#item}}<li><i class="' + '{{types}}' + '">&nbsp;</i><div class="msg-list-txt"><div><a data-id="{{_id.$id}}"  data-did="{{did.$id}}">{{msgtitle}}{{#isread}}<i class="new">新</i>{{/isread}}</a></div><span>{{outtime}}</span><i class="msgdel" data-id="{{did.$id}}"></i></div></li>{{/item}}</ul><a href="/Manage/index#settings/message?g=00">查看更多</a>';
                    } else {
                        facTpl = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                    }

                    if (resTransform.length != 0) {
                        allTpl = '<ul class="msg-list">{{#item}}<li><i class="' + '{{types}}' + '">&nbsp;</i><div class="msg-list-txt"><div><a data-id="{{_id.$id}}"  data-did="{{did.$id}}">{{msgtitle}}{{#isread}}<i class="new">新</i>{{/isread}}</a></div><span>{{outtime}}</span><i class="msgdel" data-id="{{did.$id}}"></i></div></li>{{/item}}</ul><a href="/Manage/index#settings/message?g=0">查看更多</a>';
                    } else {
                        allTpl = '<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>';
                    }

                    html = mustache.render(allTpl, {
                        item: resTransform
                    });

                    syshtml = mustache.render(sysTpl, {
                        item: sysType
                    });

                    sechtml = mustache.render(secTpl, {
                        item: secType
                    });

                    loghtml = mustache.render(logTpl, {
                        item: logType
                    });

                    fachtml = mustache.render(facTpl, {
                        item: facType
                    });
                ...
                function readMessage(url, ele, id) {
                    ...
                    $(ele).find('.time').text(msgObj[id].outtime);
                    $(ele).find('.content').html(msgObj[id].msgcontext);
                    $(ele).find('.close').on('click', function() {
                        $(ele).addClass('hide');
                    });
                    RsCore.ajax('message/readmsg', { did: tdid }, function(data, code, msg) {});
                }

                //删除消息
                view.off('click', '.msgdel').on('click', '.msgdel', function() {
                    var self = this;
                    var did = $(this).data('id');
                    bootbox.confirm('删除此消息', function(ok) {
                        if (ok) {
                            RsCore.ajax('message/delmsg', { did: did }, function(data, code, msg) {
                                if (code == 0) {
                                    RsCore.msg.success('删除成功');
                                    $(self).parents('li').remove();
                                    var ul = $(self).closest('ul');
                                    if (!ul.find('li').length) {
                                        ul.parent().html('<div class="home-msg-nomatch"><p>暂无内容~ </p><a href="/Manage/index#settings/message?g=0">查看更多</a></div>');
                                    }
                                }
                            });
                        }
            ...
            //搜索事件
            view.on('click', '.search-btn', function() {
                var val = $.trim($(this).prev().val());
                if (val == '') {
                    RsCore.msg.warn('搜索内容不能为空');
                    return;
                }
                renderHTML(val);
            });
            ...
            view.on('click', '.home-msg .opts a', function() {
                ...
                that.closest('.home-msg').find(tag).show().siblings().hide();
            });
        }
    ...
    return {
        ...
        render: function(container) {
            ...
            view.find('[downtype="windows"]').tooltip({
                html: true,
            ...
            view.find('[downtype="linux"]').tooltip({
                html: true,
            ...
            view.find('[downtype="android"]').tooltip({
                html: true,
