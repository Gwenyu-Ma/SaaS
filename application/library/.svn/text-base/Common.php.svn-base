...
class Common
{
    ...
    public static function emailShareHtml($url)
    {
        $html = "<html><head></head><body>";
        $html .= "<p>尊敬的用户，您好：</p>";
        $html .= "<b>您的好友正通过分享URL邀请你使用瑞星安全云产品：</b><br />您可以通过以下链接下载瑞星安全云终端：<br/><a href=\"$url\">$url</a><hr>";

        $html .= "如果以上链接无法点击，请将上面的地址复制到您的浏览器（如IE）的地址栏进行下载---<br/>";
        $html .= "</body></html>";
        return $html;
    }
