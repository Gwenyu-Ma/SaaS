...
class UserModel
{
     ...
     * @param 旧密码 $oldPwd
     * @param 新密码 $newPwd
     * @return bool/array 成功返回true，失败返回array
     */
    public function modifyPwd($eid, $uid, $oldPwd, $newPwd)
    {
        //验证原始密码
        ...
        if (strcasecmp($user['PWD'], Common::passMd5($oldPwd, $user['Salt']))!=0) {
            return [
                'msg' => '密码错误',
            ];
        }
        ...
        //修改新密码
        $salt = Common::saltMd5();
        $pwd = Common::passMd5($newPwd, $salt);
        $data = [
            'PWD' => $pwd,
            'Salt' => $salt,
        ];
        ...
        $result = $this->db_obj->updateTab('esm_user', $data, $where);
        if ($result >= 0) {
            add_oplog(3,9001,null,Common::passMd5($oldPwd, $user['Salt']),Common::passMd5($newPwd, $user['Salt']),'成功');
            return true;
        }
        add_oplog(3,9001,null,Common::passMd5($oldPwd, $user['Salt']),Common::passMd5($newPwd, $user['Salt']),'修改密码时发生错误');

        return ['msg' => '修改密码时发生错误'];
    }
    ...
    {
        $result= $this->db_obj->getListOne('esm_user',['imageinfo'], array(
            'locate_EID' => $eid,
            'locate_UserId' => $uid,
        ));
        ...
        $logoPath= __dir__.'/../../public/img/user.png';
