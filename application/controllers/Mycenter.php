...
class MycenterController extends MyController
{
     ...
     */
    public function eidinfoAction()
    {
        $data = (new OrganizationModel())->getOrganizationById($this->_eid);

        if (empty($data)) {
            $this->notice('获取企业信息失败', 1);
            return;
        }

        $this->ok([
            'isTrial' => $data['IsTrial'], //是否试用
            'oType' => $data['OType'], //类型
            'isOpenCenterPass' => !empty($data['PWD']), //是否启用中心密码=>0不启用，1启用
            'centerPass' => $data['PWD'],
            'company' => $data['OName'],
            'industry' => $data['Industry'],
            'companySize' => $data['OSize'],
    ...
    public function uploadlogopreviewAction()
    {
        ...
        $_SESSION['logo'] = $data;
        // $orgModel = new OrganizationModel();
        // $result = $orgModel->upLoadLogoPreview($this->_eid,$this->_uid,$token,$data);
        // if(is_bool($result)&&$result){
        $this->ok([
            'size' => $logoSize,
            'height' => $img->height(),
    ...
    public function getlogopreviewAction()
    {
        ...
        // $orgModel = new OrganizationModel();
        // $logoToken=$_SESSION['logo'];
        // $_SESSION['logo']=null;
        // echo $orgModel->getLogoPreview($this->_eid,$this->_uid,$logoToken);
    }
    ...
    private function updateUserNameByEmail()
    {
        ...
        //var_dump([$code,@$_SESSION['captcha_keystring'],strcasecmp($code , @$_SESSION['captcha_keystring'])]);
