...
class PolicyController extends MyController
{
    ...
    public function addAutoGroup()
    {
        ...
        if (is_string($rst)) {
            ...
            echo json_encode(Common::returnAjaxMsg(1, 0, $rst));
            return;
        }
        if (is_bool($rst) && !$rst) {
            add_oplog('修改','2007',null,$this->param('oldpolicyinfo'),$this->param('policyinfo'),1, '编辑入组规则失败');
            echo json_encode(Common::returnAjaxMsg(1, 0, '添加失败, 请稍候再试.'));
            return;
        }
        add_oplog('修改','2007',null,$this->param('oldpolicyinfo'),$this->param('policyinfo'),0, '编辑入组规则成功');
        echo json_encode(Common::returnAjaxMsg(0, 0, '添加成功'));
        return;
    }
    public function getAutoGroupAction()
    {
        $rules = (new AutoGroup())->getRules($this->userinfo['EID']);
        if (!is_array($rules)) {
            $rules = [];
        }
        echo json_encode(Common::returnAjaxMsg(0, 0, '', $rules));
    }
