...
use Lib\Store\MysqlCluster as MCREAD;
use Lib\Util\Log;
...
class RedisDataManager
{
    ...
    //初始化爱办公
    public static function initIoaCenterDisk($eid,$disk_set)
    {
        if(intval($disk_set) !== 0  && intval($disk_set) !==1){
            MC::clean();
            $row=MC::getRow('select disk_set from eid_orgid where eid=:eid',[':eid'=>$eid]);
            $disk_set=empty($row)? 0:$row['disk_set'];
        }
        Redis::hSet(CACHE_REDIS_ORG_PRE.$eid,'disk_set',$disk_set);
    }
    ...
    public static function isBlackGroup($eid, $groupId)
    {
        ...
        //var_dump([1, self::$BlackGUID, $groupId]);
