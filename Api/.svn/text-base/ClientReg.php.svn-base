...
use \Lib\Store\RedisCluster as reds;
use \Lib\Util\Log;
...
class ClientReg
{
	...
	WHERE U.`Status`=1 AND U.`Level`=1 AND U.LockState=1
...
//        }
	...
	INNER JOIN esm_organization O ON U.EID = O.EID
	WHERE U.`Status`=1 AND U.`Level`=1 AND U.LockState=1
	AND (U.EMail=:user OR U.UserName = :user OR U.PhoneNO=:user) AND IFNULL(O.PWD,'') =:pwd LIMIT 1 ";
    ...
    {
        $sql = " SELECT O.EID,O.OName,O.OType,O.IsTrial,O.Contact,O.Tel,O.Addr,O.ZipCode,O.CreateTime, U.UserName
